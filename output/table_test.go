...
func TestRenderTableGroupsBySourceAndShowsCounts(t *testing.T) {
	...
	out := RenderTable(tools, available, npxHistory, diff, registry.Diff{}, warnings)
...
func TestRenderDiffShowsUpdatedVersionsAndAvailability(t *testing.T) {
	diff := registry.Diff{
		Updated: []registry.ToolChange{{
			Before: model.Tool{Name: "tool", Source: model.SourceNPM, Version: "1.0.0"},
			After:  model.Tool{Name: "tool", Source: model.SourceNPM, Version: "1.1.0"},
		}},
	}
	availabilityDiff := registry.Diff{Added: []model.Tool{{Name: "gh", Source: model.SourcePath}}}

	out := RenderTable(nil, nil, nil, diff, availabilityDiff, nil)
	for _, want := range []string{
		"UPDATED",
		"~ tool (npm) 1.0.0 -> 1.1.0",
